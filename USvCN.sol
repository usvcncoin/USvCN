/**
 *Submitted for verification at BscScan.com on 2025-04-17
*/
...
contract USvCN is ERC20 {
    address public immutable taxWallet;
    uint256 public constant TAX_PERCENT = 1;

    constructor(address _founderWallet, address _communityVault) ERC20("USvCN Coin", "USvCN") {
        require(_founderWallet != address(0), "Invalid founder address");
        require(_communityVault != address(0), "Invalid vault address");

        taxWallet = _founderWallet;

        uint256 totalSupply = 1_000_000_000 * 10 ** decimals();
        uint256 founderShare = (totalSupply * 15) / 100;
        uint256 vaultShare = totalSupply - founderShare;

        _mint(_founderWallet, founderShare);
        _mint(_communityVault, vaultShare);
    }
